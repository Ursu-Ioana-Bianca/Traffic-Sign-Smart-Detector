...
# Endpoint-ul DBpedia SPARQL
endpoint = "https://dbpedia.org/sparql"

# Creăm un obiect SPARQLWrapper
sparql = SPARQLWrapper(endpoint)

# Definim query-ul SPARQL pentru Warning_sign
query = """
    PREFIX dbo: <http://dbpedia.org/ontology/>
    PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>

    SELECT ?signDescription WHERE {
        <http://dbpedia.org/resource/Warning_sign> dbo:abstract ?signDescription .

        FILTER (LANG(?signDescription) = "en")
    }
"""

# Setăm query-ul și specificăm formatul de răspuns
...
for result in results["results"]["bindings"]:
    print(f"Description: {result['signDescription']['value']}")
